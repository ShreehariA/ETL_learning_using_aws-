...
arg=getResolvedOptions(sys.argv,['s3_target_path_key','s3_target_path_bucket'])
bucket=arg['s3_target_path_bucket']
fileName=arg['s3_target_path_key']

print(bucket,fileName)

spark = SparkSession.builder.appName("CDC").getOrCreate()
inputFilePath = f"s3a://{bucket}/{fileName}"
finalFilePath = f"s3a://learning-testing/output"
...
if "LOAD" in fileName:
    ...
    ffdf.write.mode("append").csv(finalFilePath) 
